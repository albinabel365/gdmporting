...
//  DIFFERENT SCREEN CONSTRUCTION METHODS
  ...
  function createFRScreen(freeRoundsInfo,headerObj, screenType, isRejectable, callback) {
    ...
    msgBoxItems = msgBoxItems.concat(createBodyItems(freeRoundsInfo, screenType));
...
//END OF SCREEN CONSTRUCTION METHODS
  ...
  function createProgressBodyItems(freeRoundsInfo) {
    ...
    var summaryText = "You have " + freeRoundsInfo.OPTIONS[0].REMAININGROUNDS + " Free Spins.\nGood Luck!";
  ...
  function createFROptionsBody(numSpins, spinValue, betLevel) {
    ...
    // msgBodyDiv.addEventListener('mouseover', function(){msgBodyDiv.classList.add("fr-option-highlight");});
    // msgBodyDiv.addEventListener('mouseout', function(){msgBodyDiv.classList.remove("fr-option-highlight");});
  ...
  function createFRButtons(objButtons) {
    ...
    var msgButtonsDiv = createDiv(null, "commonUI-message-buttons", null, btnList);
...
var cssLoaded = false;
  ...
  function createMissionUI(message) {
        if(document.getElementById("gsIframe")){
            ...
            //gsFrame.style.height = '440px';
            //gsFrame.style.height = screen.availHeight+"px";
...
var CUIMessages = {};
   ...
   *   OPTIONS : [{
   *      ID : 0,
   *      ACTION : 'http://cb.oper.com/B4711?sid=34754',
   *      BODY : 'OK'
   *     },
   *     {
   *      ID : 103,
   *      ACTION : 'http://www.oper.com/B4711',
   *      BODY : 'Bonus Details'
   *     }
   *   ]
   *  }
